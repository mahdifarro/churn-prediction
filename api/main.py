...
from api.schemas import CustomerData, PredictionResponse, BatchPredictionRequest, BatchPredictionResponse
import logging
...
class StructuredError(Exception):
    ...
    def __init__(self, message: str, error_code: str = "INTERNAL_ERROR", trace_id: str = None):
...
try:
    ...
    latest_version_info = client.get_latest_versions(name=MODEL_NAME, stages=[MODEL_STAGE])[0]
    ...
    print(f"[DEBUG] Model URI being loaded: {model_uri}")  # DEBUG: Print to stdout for container logs
...
def predict_churn(
    ...
    threshold: float = Query(0.5, ge=0.0, le=1.0, description="Decision threshold for churn prediction")
):
    """
...
@app.post("/predict-batch", response_model=BatchPredictionResponse, summary="Batch Predict Churn")
