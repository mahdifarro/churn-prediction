...
def plot_feature_importance(pipeline, run_id, plots_session_dir):
    ...
    try:
        ...
        print("Feature importance plot is not available for this model type (e.g., Logistic Regression).")
...
def run_training():
    ...
    with mlflow.start_run(run_name="Full_Model_Comparison") as parent_run:
        ...
        for model_config in models_to_run:
            ...
            with mlflow.start_run(run_name=f"{model_name}_run", nested=True) as child_run:
                ...
                if model_name == 'LogisticRegression':
                    ...
                    model = XGBClassifier(random_state=RANDOM_STATE, use_label_encoder=False, **params)
                ...
                y_pred_cv = cross_val_predict(full_pipeline, X, y, cv=cv, method='predict')
                y_pred_proba_cv = cross_val_predict(full_pipeline, X, y, cv=cv, method='predict_proba')[:, 1]
                ...
                mlflow.sklearn.log_model(
                    ...
                    artifact_path=PIPELINE_ARTIFACT_NAME
                )
